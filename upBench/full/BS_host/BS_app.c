...
void *bs(void *args) {
	...
	struct Params p = input_params_bs(argc, argv, (end - start + 1));
	...
	//for (unsigned int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		/*if (rep >= p.n_warmup)
		bs_start(&timer, 1, rep - p.n_warmup);*/
		...
		for(int id = start; id <= end; id++)
		{
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], input));
		}

		for(int id = start; id <= end; id++)
			DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, input_size * sizeof(DTYPE), DPU_XFER_DEFAULT));

		i = 0;

		//DPU_FOREACH(dpu_set, dpu, i)
		for(int id = start; id <= end; i++, id++)
		{
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], querys + slice_per_dpu * i));
		}

		for(int id = start; id <= end; id++)
			DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, input_size * sizeof(DTYPE), slice_per_dpu * sizeof(DTYPE), DPU_XFER_DEFAULT));
		...
		/*if (rep >= p.n_warmup)
		{
			bs_start(&timer, 2, rep - p.n_warmup);
			#if ENERGY
	...
	/*	DPU_ASSERT(dpu_launch(dpu_set, DPU_SYNCHRONOUS));
		...
		// Retrieve results
		if (rep >= p.n_warmup)
		bs_start(&timer, 3, rep - p.n_warmup);
	...
	bs_print(&timer, 0, p.n_reps);
	printf("CPU-DPU Time (ms): ");
	bs_print(&timer, 1, p.n_reps);
	printf("DPU Kernel Time (ms): ");
	bs_print(&timer, 2, p.n_reps);
	printf("DPU-CPU Time (ms): ");
	bs_print(&timer, 3, p.n_reps);
