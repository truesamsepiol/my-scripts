...
void trns(int start, int end) {
    ...
    //for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		for(int id = start; id <= end; id++){
                    DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], &A_backup[j * N_ * n + n * (i + curr_dpu)]));
                    i++;
                }
                for(int id = start; id <= end; id++)
			DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, sizeof(T) * j * n, sizeof(T) * n, DPU_XFER_DEFAULT));
            }
            /*if(rep >= p.n_warmup)
                trns_stop(&timer, 1);*/
            ...
            //DPU_FOREACH(dpu_set, dpu) {
	    for(int id = start; id <= end; id++){
                DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], done_host));
            }
            for(int id = start; id <= end; id++)
		    DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, M_ * m * n * sizeof(T), (M_ * n) / 8 == 0 ? 8 : M_ * n, DPU_XFER_DEFAULT));
...
#if PRINT
        {
            ...
            // Run DPU kernel
            if(rep >= p.n_warmup){
                trns_start(&timer, 3, rep - p.n_warmup + timer_fix);
#if ENERGY
                DPU_ASSERT(dpu_probe_start(&probe));
#endif
            }
...
#if PRINT
        {
            ...
            if(rep >= p.n_warmup)
                trns_start(&timer, 4, rep - p.n_warmup + timer_fix);
            DPU_FOREACH(dpu_set, dpu) {
    ...
    printf("CPU ");
    trns_print(&timer, 0, p.n_reps);
    printf("CPU-DPU (Step 1) ");
    trns_print(&timer, 1, p.n_reps);
    printf("Step 2 ");
    trns_print(&timer, 2, p.n_reps);
    printf("Step 3 ");
    trns_print(&timer, 3, p.n_reps);
    printf("DPU-CPU ");
    trns_print(&timer, 4, p.n_reps);
