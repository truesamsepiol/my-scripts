...
void *bfs(void *args) {
    ...
    uint32_t numDPUs;
    //DPU_ASSERT(dpu_alloc((end - start + 1), NULL, &dpu_set));
    ...
    for(int id = start; id <= end; id++){
        ...
        if(dpuNumNodes > 0) {
            ...
            startTimer(&timer);
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)dpuNodePtrs_h, dpuNodePtrs_m, (dpuNumNodes + 1)*sizeof(uint32_t));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)dpuNeighborIdxs_h, dpuNeighborIdxs_m, dpuNumNeighbors*sizeof(uint32_t));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)dpuNodeLevel_h, dpuNodeLevel_m, dpuNumNodes*sizeof(uint32_t));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)visited, dpuVisited_m, numNodes/64*sizeof(uint64_t));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)nextFrontier, dpuNextFrontier_m, numNodes/64*sizeof(uint64_t));
        ...
        startTimer(&timer);
        copyToDPU(sub_set_of_dpus[id], (uint8_t*)&dpuParams[dpuIdx], dpuParams_m[dpuIdx], sizeof(struct DPUParams));
        stopTimer(&timer);
        loadTime += getElapsedTime(timer);
    ...
    /* EO --> Not intresting for the rest
