...
void *mlp(void *args) {
	...
	//for (unsigned int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		//DPU_FOREACH(dpu_set, dpu, i) {
		for(int id = start; id <= end; id++, i++){
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], A[0] + dpu_info[i].prev_rows_dpu * n_size));
		}
		for(int id = start; id <= end; id++)
		...
		/*if (rep >= p.n_warmup)
		{
			mpl_start(&timer, 2, rep - p.n_warmup);
#if ENERGY
		...
		for(int lay = 1; lay < NUM_LAYERS; lay++){
			if (rep >= p.n_warmup)
				mpl_start(&timer, 4, rep - p.n_warmup);
			i = 0;
		...
		// Retrieve results
		if (rep >= p.n_warmup)
			mpl_start(&timer, 3, rep - p.n_warmup);
		i = 0;
		DPU_FOREACH(dpu_set, dpu, i) {
			DPU_ASSERT(dpu_prepare_xfer(dpu, C_dpu + i * max_rows_per_dpu));
		}
	...
	mpl_print(&timer, 1, p.n_reps);
	printf("DPU Kernel Time (ms): ");
	mpl_print(&timer, 2, p.n_reps);
	printf("Inter-DPU Time (ms): ");
	mpl_print(&timer, 4, p.n_reps);
	printf("DPU-CPU Time (ms): ");
	mpl_print(&timer, 3, p.n_reps);
