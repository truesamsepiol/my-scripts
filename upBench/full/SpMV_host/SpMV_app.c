...
#include "../SpMV_support/common.h"
#include "../SpMV_support/matrix.h"
#include "../SpMV_support/params.h"
    ...
    //DPU_FOREACH (dpu_set, dpu) {
      for(int id = start; id <= end; id++){
        ...
        if(dpuNumRows > 0) {
            ...
            spmv_startTimer(&timer);
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)dpuRowPtrs_h, dpuRowPtrs_m, (dpuNumRows + 1)*sizeof(uint32_t));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)dpuNonzeros_h, dpuNonzeros_m, dpuNumNonzeros*sizeof(struct Nonzero));
            copyToDPU(sub_set_of_dpus[id], (uint8_t*)inVector, dpuInVector_m, numCols*sizeof(float));
            spmv_stopTimer(&timer);
        ...
        spmv_startTimer(&timer);
        copyToDPU(sub_set_of_dpus[id], (uint8_t*)&dpuParams[dpuIdx], dpuParams_m, sizeof(struct DPUParams));
        spmv_stopTimer(&timer);
        loadTime += getElapsedTime(timer);
