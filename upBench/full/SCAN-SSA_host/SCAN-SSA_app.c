...
#include "../SCAN-SSA_support/common.h"
...
void *scan_ssa(void *args) {
    ...
    //for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        /*if(rep >= p.n_warmup)
            scan_ssa_start(&timer, 0, rep - p.n_warmup);*/
        scan_host(C, A, input_size);
        /*if(rep >= p.n_warmup)
            scan_ssa_stop(&timer, 0);*/

        printf("Load input data\n");
        /*if(rep >= p.n_warmup)
            scan_ssa_start(&timer, 1, rep - p.n_warmup);*/
        // Input arguments
	...
	for(int id = start; id <= end; id++){
            DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], &input_arguments));
        }
        for(int id = start; id <= end; id++)
		DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, "DPU_INPUT_ARGUMENTS", 0, sizeof(input_arguments), DPU_XFER_DEFAULT));
        //DPU_FOREACH(dpu_set, dpu, i) {
	for(int id = start; id <= end; id++, i++){
            DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferA + input_size_dpu * i));
        }
        for(int id = start; id <= end; id++)
		DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, input_size_dpu * sizeof(T), DPU_XFER_DEFAULT));
        /*if(rep >= p.n_warmup)
            scan_ssa_stop(&timer, 1);*/

        printf("Run program on DPU(s) \n");
        // Run DPU kernel
        /*if(rep >= p.n_warmup) {
            scan_ssa_start(&timer, 2, rep - p.n_warmup);
            #if ENERGY
        ...
        if(rep >= p.n_warmup)
            scan_ssa_start(&timer, 3, rep - p.n_warmup);
        ...
        if(rep >= p.n_warmup)
            scan_ssa_stop(&timer, 3);

        printf("Run program on DPU(s) \n");
        // Run DPU kernel
        if(rep >= p.n_warmup) {
            scan_ssa_start(&timer, 4, rep - p.n_warmup);
            #if ENERGY
            DPU_ASSERT(dpu_probe_start(&probe));
            #endif
        }
        ...
        printf("Retrieve results\n");
        if(rep >= p.n_warmup)
            scan_ssa_start(&timer, 5, rep - p.n_warmup);
        i = 0;
        // PARALLEL RETRIEVE TRANSFER
        DPU_FOREACH(dpu_set, dpu, i) {
    ...
    printf("CPU ");
    scan_ssa_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    scan_ssa_print(&timer, 1, p.n_reps);
    printf("DPU Kernel Scan ");
    scan_ssa_print(&timer, 2, p.n_reps);
    printf("Inter-DPU (Scan) ");
    scan_ssa_print(&timer, 3, p.n_reps);
    printf("DPU Kernel Add ");
    scan_ssa_print(&timer, 4, p.n_reps);
    printf("DPU-CPU ");
    scan_ssa_print(&timer, 5, p.n_reps);
