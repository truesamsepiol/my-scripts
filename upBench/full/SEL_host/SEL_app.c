...
#include "../SEL_support/common.h"
#include "../SEL_support/timer.h"
#include "../SEL_support/params.h"
#include "../header.h"

// Define the DPU Binary path as DPU_BINARY here
#ifndef DPU_BINARY
#define DPU_BINARY "./SEL_bin/dpu_code"
#endif
...
void *sel(void *args) {
	...
	for(int id = start; id <= end; id++, i++){
        ...
        // Run DPU kernel
        if(rep >= p.n_warmup) {
            sel_start(&timer, 2, rep - p.n_warmup);
            #if ENERGY
        ...
        if(rep >= p.n_warmup)
		    sel_start(&timer, 3, rep - p.n_warmup);
        ...
        i = 0;
        if(rep >= p.n_warmup)
            sel_start(&timer, 4, rep - p.n_warmup);
        DPU_FOREACH (dpu_set, dpu) {
            // Copy output array
    ...
    printf("CPU ");
    sel_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    sel_print(&timer, 1, p.n_reps);
    printf("DPU Kernel ");
    sel_print(&timer, 2, p.n_reps);
    printf("Inter-DPU ");
    sel_print(&timer, 3, p.n_reps);
    printf("DPU-CPU ");
    sel_print(&timer, 4, p.n_reps);
