...
void va(int start, int end){
	...
	for(int id = start; id <= end; id++, i++){
            DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], &input_arguments[i]));
        }
        for(int id = start; id <= end; id++)
		DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, "DPU_INPUT_ARGUMENTS", 0, sizeof(input_arguments[0]), DPU_XFER_DEFAULT));

        //DPU_FOREACH(dpu_set, dpu, i) {
	for(int id = start, i = 0; id <= end; id++, i++){
            DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferA + input_size_dpu_8bytes * i));
        }
        for(int id = start; id <= end; id++)
		DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, input_size_dpu_8bytes * sizeof(T), DPU_XFER_DEFAULT));
 
        //DPU_FOREACH(dpu_set, dpu, i) {
	for(int id = start, i = 0; id <= end; id++, i++){
            DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferB + input_size_dpu_8bytes * i));
        }
        for(int id = start; id <= end; id++)
		DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, input_size_dpu_8bytes * sizeof(T), input_size_dpu_8bytes * sizeof(T), DPU_XFER_DEFAULT));
        /*if(rep >= p.n_warmup)
            va_stop(&timer, 1);
        ...
        // Run DPU kernel
        if(rep >= p.n_warmup) {
            va_start(&timer, 2, rep - p.n_warmup);
            #if ENERGY
        ...
        if(rep >= p.n_warmup)
            va_start(&timer, 3, rep - p.n_warmup);
        i = 0;
    ...
    printf("CPU ");
    va_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    va_print(&timer, 1, p.n_reps);
    printf("DPU Kernel ");
    va_print(&timer, 2, p.n_reps);
    printf("DPU-CPU ");
    va_print(&timer, 3, p.n_reps);
