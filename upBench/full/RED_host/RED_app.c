...
void red(int start, int end) {
	...
	for(int id = start, i = 0; id <= end; id++, i++){
        ...
        if(rep >= p.n_warmup)
            red_start(&timer, 3, rep - p.n_warmup);
        i = 0;
    ...
    printf("CPU ");
    red_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    red_print(&timer, 1, p.n_reps);
    printf("DPU Kernel ");
    red_print(&timer, 2, p.n_reps);
    printf("Inter-DPU ");
    red_print(&timer, 3, p.n_reps);
