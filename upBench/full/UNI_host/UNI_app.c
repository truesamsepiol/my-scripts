...
void uni(int start, int end) {
    ...
    //for(int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
        ...
        //if(rep >= p.n_warmup)
            //uni_start(&timer, 0, rep - p.n_warmup);
        ...
        //if(rep >= p.n_warmup)
          //  uni_start(&timer, 1, rep - p.n_warmup);
        // Input arguments
	...
	for(int id = start; id <= end; id++, i++){
        ...
        // Run DPU kernel
        //if(rep >= p.n_warmup) {
          //  uni_start(&timer, 2, rep - p.n_warmup);
           // #if ENERGY
       ...
       // }
        ...
        if(rep >= p.n_warmup)
            uni_start(&timer, 3, rep - p.n_warmup);
        ...
        i = 0;
        if(rep >= p.n_warmup)
            uni_start(&timer, 4, rep - p.n_warmup);
        DPU_FOREACH (dpu_set, dpu) {
            // Copy output array
    ...
    printf("CPU ");
    uni_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    uni_print(&timer, 1, p.n_reps);
    printf("DPU Kernel ");
    uni_print(&timer, 2, p.n_reps);
    printf("Inter-DPU ");
    uni_print(&timer, 3, p.n_reps);
    printf("DPU-CPU ");
    uni_print(&timer, 4, p.n_reps);
