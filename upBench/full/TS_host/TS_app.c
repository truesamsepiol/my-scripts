...
void *ts(void *args){
	...
	//for (int rep = 0; rep < p.n_warmup + p.n_reps; rep++) {
		...
		for(int id = start; id <= end; id++){
			...
			DPU_ASSERT(dpu_copy_to(sub_set_of_dpus[id], "DPU_INPUT_ARGUMENTS", 0, (const void *) &input_arguments, sizeof(input_arguments)));
			i++;
		}
		...
		for(int id = start; id <= end; id++)
		{
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferQ));
		}

		for(int id = start; id <= end; id++)
			DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, 0, query_length * sizeof(DTYPE), DPU_XFER_DEFAULT));
		...
		for(int id = start; id <= end; id++, i++){
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferTS + slice_per_dpu * i));
		}

		for(int id = start; id <= end; id++)
			DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, mem_offset,(slice_per_dpu + query_length)*sizeof(DTYPE), DPU_XFER_DEFAULT));
		...
		//DPU_FOREACH(dpu_set, dpu, i) {
		for(int id = start; id <= end; id++, i++){
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferAMean + slice_per_dpu * i));
		}
		...
		//DPU_FOREACH(dpu_set, dpu, i) {
		for(int id = start; id <= end; id++, i++){
			DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], bufferASigma + slice_per_dpu * i));
		}
		...
		if (rep >= p.n_warmup)
		{
			ts_start(&timer, 2, rep - p.n_warmup);
#if ENERGY
		...
		if (rep >= p.n_warmup)
			ts_start(&timer, 3, rep - p.n_warmup);
		...
		if (rep >= p.n_warmup)
			ts_start(&timer, 4, rep - p.n_warmup);
	...
	ts_print(&timer, 4, p.n_reps);
	printf("Inter-DPU Time (ms): ");
	ts_print(&timer, 0, p.n_reps);
	printf("CPU-DPU Time (ms): ");
	ts_print(&timer, 1, p.n_reps);
	printf("DPU Kernel Time (ms): ");
	ts_print(&timer, 2, p.n_reps);
	printf("DPU-CPU Time (ms): ");
	ts_print(&timer, 3, p.n_reps);
