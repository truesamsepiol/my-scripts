...
#ifndef DPU_BINARY
#define DPU_BINARY "./NW_bin/nw_dpu"
#endif
...
/*#if DYNAMIC 
	    ...
	    for(int id = start; id <= end; id++, i++){
            ...
            /*if (rep >= p.n_warmup) {
                if ((max_cols-1)/BL == 1) 
                    ...
                    nw_start(&timer, 1, rep - p.n_warmup + blk - 1);
		    ...
		    for(int id = start; id <= end; id++, i++){
                        ...
                        DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], dpu_pointer + input_itemsets_offset));
                    }

                    if (bl == 0)
			    for(int id = start; id <= end; id++)
                        	DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, mram_offset, (BL+2) * sizeof(int32_t), DPU_XFER_DEFAULT));
                    else
			    for(int id = start; id <= end; id++)
                        	DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, mram_offset, 2 * sizeof(int32_t), DPU_XFER_DEFAULT));
		    ...
		    for(int id = start; id <= end; id++, i++){
                        ...
                        DPU_ASSERT(dpu_prepare_xfer(sub_set_of_dpus[id], dpu_pointer + reference_offset));
                    }
		    for(int id = start; id <= end; id++)
                    	DPU_ASSERT(dpu_push_xfer(sub_set_of_dpus[id], DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, mram_offset, BL * sizeof(int32_t), DPU_XFER_DEFAULT));
...
#endif
            if (rep >= p.n_warmup) {
                nw_start(&timer, 3, rep - p.n_warmup + blk - 1);
                // Timer for longest diagonal
                if (blk == ((max_cols-1)/BL)) {
            ...
            if (rep >= p.n_warmup) {
                nw_start(&timer, 4, rep - p.n_warmup + blk - 1);
                // Timer for longest diagonal
                if (blk == ((max_cols-1)/BL)) {
                    nw_start(&long_diagonal_timer, 4, rep - p.n_warmup);
                }
            }
            // Retrieve results
...
#if ENERGY
            if (rep >= p.n_warmup) {
                DPU_ASSERT(dpu_probe_nw_start(&probe));
            }
#endif
            if (rep >= p.n_warmup)
...
#if ENERGY
            if (rep >= p.n_warmup) {
                DPU_ASSERT(dpu_probe_nw_stop(&probe));
            }
#endif
    ...
    printf("CPU version ");
    nw_print(&timer, 0, p.n_reps);
    printf("CPU-DPU ");
    nw_print(&timer, 2, p.n_reps);
    printf("DPU Kernel ");
    nw_print(&timer, 3, p.n_reps);
    printf("Inter-DPU ");
    nw_print(&timer, 1, p.n_reps);
    printf("DPU-CPU ");
    nw_print(&timer, 4, p.n_reps);
    printf("\n");
    printf("Longest Diagonal CPU-DPU ");
    nw_print(&long_diagonal_timer, 2, p.n_reps);
    printf("Longest Diagonal DPU Kernel ");
    nw_print(&long_diagonal_timer, 3, p.n_reps);
    printf("Longest Diagonal Inter-DPU ");
    nw_print(&long_diagonal_timer, 1, p.n_reps);
    printf("Longest Diagonal DPU-CPU ");
    nw_print(&long_diagonal_timer, 4, p.n_reps);
    printf("\n");
