...
void *check_dpus_running(void *arg){
	while(true){
		...
		printf("%ldms, %d/%d are running %d are wasted\n", sec, nr_dpus_are_running, NR_DPUS_MAX_SIMULATION, NR_DPUS_MAX_SIMULATION - nr_dpus_are_running);
...
void full_scheduler(){
	...
	while(j < tour){
		...
		while(j < tour){
			...
			if(nr_dpus_total + atoi(programs[j].program[2]) <= NR_DPUS_MAX_SIMULATION)
			{
		...
		int end, start = 0;
		pthread_t threads[NR_DPUS_MAX_SIMULATION];
		struct args threads_args[NR_DPUS_MAX_SIMULATION];
